...
def internal_reset():
    ...
    global current_chunk_index, record_start_time, next_split_time, chunk_split_requested
    global buffer
...
def stop_recording_and_transcribe():
    ...
    if active_filename and os.path.exists(active_filename):
        if os.path.getsize(active_filename) > 44:  # Basic WAV header size
    ...
    # 6) Reset internal state AFTER everything is complete
    internal_reset()

def setup_hotkeys():
    """
    Set up keyboard hooks with proper cleanup
    """
...
def startup():
    # Clean up any existing keyboard hooks
    keyboard.unhook_all()
    
    # Set up fresh hotkeys
    setup_hotkeys()
