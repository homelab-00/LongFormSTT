...
# Use full precision (float32) to potentially improve accuracy.
model = WhisperModel(model_id, device=device, compute_type="float32")
...
def record_audio():
    ...
    try:
        while recording:
            ...
            # Accumulate speech frames in our write buffer
            if speech_frames:
                buffer.append(b"".join(speech_frames))
            ...
            # If we've at least hit MIN_CHUNK_LENGTH_SEC, we watch for a short silence.
            # That means: if we had *no speech* in this 64ms chunk, increment silence_frame_count
            # else reset to 0.
