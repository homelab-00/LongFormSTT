...
keyboard.add_hotkey('F5', start_transcription, suppress=True)


def stop_recording():
    """Stops the recording and prepares for transcription."""
    global recording, active_wave_file
    ...
    # Close active wave file, if it exists and is not already closed
    if active_wave_file:
        try:
            active_wave_file.close()
        except Exception as e:
            console.print(f"[red]Failed to close the active wave file: {e}[/red]")
