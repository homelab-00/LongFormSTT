...
class AudioTranscriber:
    ...
    def stop_recording(self):
        ...
        self.recording = False
        self._finalize_recording()  # This method needs to exist
        self._process_remaining_chunk()  # Now properly defined
    ...
    def _process_remaining_chunk(self):
        ...
        if self.active_wavefile and self.active_wavefile.name:
            filename = self.active_wavefile.name
            if os.path.exists(filename) and os.path.getsize(filename) > 44:
                self._transcribe_chunk(filename)
    ...
    def _write_buffer(self):
        ...
        if self.buffer and self.active_wavefile:  # ADDED NULL CHECK
            try:
    ...
    def _split_chunk(self):
        ...
        current_filename = os.path.join(self.script_dir, f"temp_audio_file{self.current_chunk}.wav")
        self._close_current_wavefile()
        self._transcribe_chunk(current_filename)
        self.current_chunk += 1
        self._create_wave_file()
...
def _process_remaining_chunk(self):
    """Process the final incomplete chunk"""
    if self.active_wavefile and os.path.exists(self.active_wavefile.name):
        if os.path.getsize(self.active_wavefile.name) > 44:
