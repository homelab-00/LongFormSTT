...
class Config:
    # Audio settings
    use_system_audio: bool = False                # For long-form STT (default to microphone)
    ...
    # Transcription settings for Long-Form STT
    language: str = "el"                          # Long-form language (default: Greek)
    task: str = "transcribe"                      # Long-form task
    
    # Transcription settings for Real-Time STT
    realtime_language: str = "en"                 # Real-time language (default: English)
    realtime_task: str = "transcribe"             # Real-time task
...
class RealtimeTranscriptionHandler:
    ...
    def set_audio_source(self, use_system_audio):
        """Set the audio source for real-time transcription without starting it."""
        was_running = self.is_running
        if was_running:
            self.stop()
        
        self.config.realtime_use_system_audio = use_system_audio
        source_str = "system audio" if self.config.realtime_use_system_audio else "microphone"
        self.console.print(f"[cyan]Set real-time audio source to: {source_str}[/cyan]")
        
        # Update the tray icon for visual feedback
        self.tray.flash_white('gray', self.config.send_enter)
                        ...
                        if self.is_speech_active:
                            ...
                            if (current_time - silence_start_time) * 1000 >= self.silence_threshold_ms:
                                ...
                                if current_segment:
                                    ...
                                    # Set task based on language - always translate non-Greek/non-English to English
...
class CommandServer:
    ...
    def _handle_command(self, command: str) -> None:
        ...
        elif command == "OPEN_LANGUAGE_SETTINGS":
            self.app.open_language_settings()
        elif command == "OPEN_AUDIO_SETTINGS":
            self.app.open_audio_settings()
        elif command == "QUIT":
...
class STTApp:
    ...
    def _display_info(self) -> None:
        ...
        panel_content = (
            ...
            " F7  -> Quit\n"
            " F8  -> Toggle real-time transcription (single-tap) / Toggle long-form audio source (double-tap)\n"
            " F9  -> Toggle real-time audio source\n"
            " F10 -> Static file transcription\n"
            f"[bold yellow]Language[/bold yellow]: {self.config.language} (Task: {self.config.task})\n"
            f"[bold yellow]Long-form Audio Source[/bold yellow]: {'System Audio' if self.config.use_system_audio else 'Microphone'}\n"
            f"[bold yellow]Real-time Audio Source[/bold yellow]: {'System Audio' if self.config.realtime_use_system_audio else 'Microphone'}"
        )
        panel = Panel(panel_content, title="Information", border_style="green")
        self.console.print(panel)
    
    def open_language_settings(self) -> None:
        """Open the language settings dialog with separate settings for Long Form and Real Time STT."""
        self.console.print("[bold yellow]OPEN_LANGUAGE_SETTINGS command received[/bold yellow]")
        ...
        self._show_language_settings_dialog()
        
        # Resume real-time transcription if it was running
        if realtime_was_running:
            self.realtime_handler.start()
    
    def _show_language_settings_dialog(self):
        """Show a dialog for selecting language settings for both Long Form and Real Time STT."""
        # Create the main dialog window
        root = tkinter.Tk()
        root.title("Language Settings")
        ...
        header_frame = tkinter.Frame(root, bg=bg_color)
        header_frame.pack(pady=(20, 5), padx=20, fill="x")
        ...
        longform_header.grid(row=0, column=0, padx=20)
        ...
        realtime_header.grid(row=0, column=1, padx=20)
        
        # Frame for language selection
        lang_frame = tkinter.Frame(root, bg=bg_color)
        lang_frame.pack(pady=10, padx=20, fill="x")
        
        # Current language labels
        longform_lang_label = tkinter.Label(
            lang_frame, 
        ...
        longform_lang_label.grid(row=0, column=0, sticky="w", padx=20, pady=5)
        
        realtime_lang_label = tkinter.Label(
            lang_frame, 
            text="Current Language:", 
            bg=bg_color, 
            fg=text_color, 
        ...
        realtime_lang_label.grid(row=0, column=1, sticky="w", padx=20, pady=5)
        
        # Current language values
        longform_curr_name = self.language_selector.languages.get(self.config.language, "Unknown")
        longform_curr_label = tkinter.Label(
            lang_frame, 
            text=f"{longform_curr_name.title()} ({self.config.language})", 
            bg=bg_color, 
            fg=text_color, 
            font=("Arial", 10, "italic")
        )
        longform_curr_label.grid(row=1, column=0, sticky="w", padx=20, pady=2)
        
        realtime_curr_name = self.language_selector.languages.get(self.config.realtime_language, "Unknown")
        realtime_curr_label = tkinter.Label(
            lang_frame, 
            text=f"{realtime_curr_name.title()} ({self.config.realtime_language})", 
            bg=bg_color, 
            fg=text_color, 
            font=("Arial", 10, "italic")
        )
        realtime_curr_label.grid(row=1, column=1, sticky="w", padx=20, pady=2)
        ...
        longform_search_label.grid(row=2, column=0, sticky="w", padx=20, pady=(15, 5))
        ...
        realtime_search_label.grid(row=2, column=1, sticky="w", padx=20, pady=(15, 5))
        ...
        longform_search_entry.grid(row=3, column=0, sticky="ew", padx=20, pady=5)
        ...
        realtime_search_entry.grid(row=3, column=1, sticky="ew", padx=20, pady=5)
        ...
        # Long Form listbox
        longform_listbox_frame = tkinter.Frame(listbox_frame, bg=bg_color)
        longform_listbox_frame.grid(row=0, column=0, padx=20, sticky="nsew")
        ...
        # Real Time listbox
        realtime_listbox_frame = tkinter.Frame(listbox_frame, bg=bg_color)
        realtime_listbox_frame.grid(row=0, column=1, padx=20, sticky="nsew")
        ...
        # Populate listboxes with all languages
        sorted_display = sorted(self.language_selector.language_display.values())
        for display_name in sorted_display:
        ...
        # Function to update Long Form listbox based on search
        def update_longform_listbox(*args):
            search_text = longform_search_var.get().lower()
            longform_listbox.delete(0, tkinter.END)
            
            matching_codes = [code for code, name in self.language_selector.languages.items() 
                             if search_text in code.lower() or search_text in name.lower()]
            
            for code in matching_codes[:30]:
                longform_listbox.insert(tkinter.END, self.language_selector.language_display[code])
            
            if matching_codes:
                longform_listbox.selection_set(0)
        
        # Function to update Real Time listbox based on search
        def update_realtime_listbox(*args):
            ...
            for code in matching_codes[:30]:
                realtime_listbox.insert(tkinter.END, self.language_selector.language_display[code])
        ...
        # Function to extract language code from display name
        def get_language_code(display_name):
            try:
                code = display_name.split("(")[1].split(")")[0]
                return code
            except (IndexError, Exception) as e:
                self.console.print(f"[red]Selection error: {e}[/red]")
                return None
        ...
        def on_select():
            try:
                ...
                if longform_listbox.curselection():
                    longform_idx = longform_listbox.curselection()[0]
                    longform_selection = longform_listbox.get(longform_idx)
                    result["longform_code"] = get_language_code(longform_selection)
                
                # Get Real Time selection
                if realtime_listbox.curselection():
                    realtime_idx = realtime_listbox.curselection()[0]
                    realtime_selection = realtime_listbox.get(realtime_idx)
                    result["realtime_code"] = get_language_code(realtime_selection)
        ...
        # Bind double-click to select
        longform_listbox.bind("<Double-1>", lambda e: on_select())
        realtime_listbox.bind("<Double-1>", lambda e: on_select())
        ...
        if result["longform_code"]:
            ...
            self.config.language = result["longform_code"]
            longform_name = self.language_selector.languages.get(result["longform_code"], "Unknown")
            ...
            self.console.print(f"[yellow]Long Form language changed from {old_lang} to {result['longform_code']} ({longform_name}){task_msg}[/yellow]")
        
        if result["realtime_code"]:
            old_lang = self.config.realtime_language
            old_task = self.config.realtime_task
            
            self.config.realtime_language = result["realtime_code"]
            realtime_name = self.language_selector.languages.get(result["realtime_code"], "Unknown")
            
            # Auto-switch task based on language
            if result["realtime_code"] not in ["en", "el"]:
                self.config.realtime_task = "translate"
            else:
                self.config.realtime_task = "transcribe"
            
            task_msg = ""
            if old_task != self.config.realtime_task:
                task_msg = f" and task from '{old_task}' to '{self.config.realtime_task}'"
                
            self.console.print(f"[yellow]Real Time language changed from {old_lang} to {result['realtime_code']} ({realtime_name}){task_msg}[/yellow]")
