...
# For beep sounds on Windows (with gentler beep)
...
def start_recording():
    ...
    if sys.platform.startswith("win"):
        try:
            # Gentler beep: lower freq + short duration
            console.print("[dim green]Playing start-recording beep...[/dim green]")
            winsound.Beep(220, 120)  # freq=220 Hz, 120ms
...
def startup():
    ...
    # 1) Kill any leftover AHK processes referencing our script
    kill_leftover_ahk()

    # 2) Gather all existing AHK processes *after* that
    pre_pids = set()
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
    ...
    # 4) Short sleep, then see if there's exactly one new AHK process
    time.sleep(1.0)
    post_pids = set()
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
