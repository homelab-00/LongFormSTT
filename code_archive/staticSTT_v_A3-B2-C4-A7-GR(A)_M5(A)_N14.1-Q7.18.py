...
RED_ICON = None
BLUE_ICON = None
YELLOW_ICON = None  # [MOD] added a YELLOW icon
...
def init_tray_icons():
    ...
    YELLOW_ICON = create_circle_icon(size, (255,255,0,255), (0,0,0,255))  # bright yellow fill
...
def set_tray_icon_color(color_name):
    ...
    elif color_name == 'yellow':  # [MOD] handle yellow
...
def stop_tray_icon():
    """
    If the tray_icon is running, stop it so the script can exit cleanly.
    """
    global tray_icon
...
def transcribe_static_file():
    ...
    # [MOD] turn tray icon yellow for the duration
    ...
    if not file_path:
        ...
        set_tray_icon_color('gray')  # revert if user canceled
        return
    ...
    if not wav_path or not os.path.exists(wav_path):
        ...
        set_tray_icon_color('gray')  # revert icon
        return
    ...
    try:
        ...
        # [MOD] Instead of copying to clipboard, save .txt next to the original audio
