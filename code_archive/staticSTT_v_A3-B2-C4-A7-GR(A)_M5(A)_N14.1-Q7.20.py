...
# [MOD] Remove webrtcvad code, add YAMNet-based filtering
...
# --------------------------------------------------------------------------------------
# Convert or copy to 16k/mono WAV
# --------------------------------------------------------------------------------------
...
def apply_yamnet_filter(in_wav_path, speech_threshold=0.3):
    ...
    try:
        ...
        # 2) Load the YAMNet model from TF Hub
        console.print("[yellow]Loading YAMNet from TF Hub...[/yellow]")
        yamnet_model = hub.load("https://tfhub.dev/google/yamnet/1")

        # 3) Run inference
        # YAMNet processes the entire waveform at once, returns:
        # scores (Nframes x 521), embeddings, log_mel_spectrogram
        # We'll feed data as float32
        waveform = tf.constant(data, tf.float32)
        # Expand dims for single batch
        waveform = tf.reshape(waveform, [1, -1])
        scores, embeddings, spectrogram = yamnet_model(waveform)
        scores = scores.numpy()[0]  # shape (num_frames, 521)
        # The model frames are ~0.96 seconds each at 16k
        # We need the index for 'Speech'
        # Official YAMNet label list: https://github.com/tensorflow/models/tree/master/research/audioset/yamnet
        # Usually 'Speech' is label index 0 in the yamnet_class_map.csv
        # We'll assume it's index=0. Let's check though:
        # Actually, checking the official map, 'Speech' is index=0 indeed.

        speech_index = 0
        # 4) We'll gather the frames where 'Speech' >= threshold
        keep_mask = scores[:, speech_index] >= speech_threshold
        # keep_mask is shape (num_frames,)

        if not any(keep_mask):
            console.print("[red]No speech frames found by YAMNet. Returning original.[/red]")
            return in_wav_path

        # 5) Reconstruct audio from kept frames
        # Each YAMNet frame is 0.96s. We'll re-chunk the original data accordingly.
        frame_size = int(0.96 * sr)  # ~15360 for sr=16000
        # But YAMNet might do some padding/truncation. Let's see how many frames we have
        n_frames = scores.shape[0]
        total_samples = len(data)

        # The frames from YAMNet are sequential in the original. We'll map them.
        # We'll keep all samples for a frame if keep_mask[i] is True.
        # We'll do a simpler approach:
        #   chunk_data = data[i * frame_size : min((i+1)*frame_size, total_samples)]
        # Then we keep or discard it.
        ...
        for i in range(n_frames):
            ...
            if keep_mask[i]:
                final_array.append(chunk)

        if not final_array:
            console.print("[red]All frames were filtered out after bounding. Returning original.[/red]")
            return in_wav_path

        voice_data = tf.concat([tf.constant(ch, dtype=tf.float32) for ch in final_array], axis=0).numpy()
...
def transcribe_static_file():
    ...
    # 2) YAMNet filter to remove music => temp_static_voice_only.wav
