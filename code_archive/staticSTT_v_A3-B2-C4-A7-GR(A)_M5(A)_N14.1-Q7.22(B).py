...
# --------------------------------------------------------------------------------------
def apply_yamnet_filter(in_wav_path, speech_threshold=0.1):
    """
    1) Load YAMNet from TF Hub
    2) Classify each ~0.96s frame
    3) Keep frames that have 'Speech' >= speech_threshold
       plus a small buffer around high-confidence speech frames
    4) Return path to 'temp_static_voice_only.wav'
    """
    ...
    try:
        ...
        # 'Speech' is index 0
        speech_index = 0
        raw_confidences = scores[:, speech_index]  # shape => (num_frames,)

        # We'll keep frames if speech confidence >= speech_threshold
        # But also keep a buffer (1 frame on each side) around frames that pass threshold.
        keep_mask = raw_confidences >= speech_threshold

        # Expand the mask by 1 frame on each side if possible
        expanded_mask = keep_mask.copy()
        for i in range(len(keep_mask)):
            if keep_mask[i]:
                if i > 0:
                    expanded_mask[i-1] = True
                if i < len(keep_mask) - 1:
                    expanded_mask[i+1] = True

        # If no frames are kept, fallback
        if not any(expanded_mask):
            console.print("[red]No speech frames found by YAMNet. Returning original.[/red]")
            return in_wav_path

        # YAMNet frames ~0.96s for 16k input
        frame_size = int(0.96 * sr)
        total_samples = len(data)
        n_frames = len(raw_confidences)
        ...
        voice_data = tf.concat(
            [tf.constant(ch, dtype=tf.float32) for ch in result_chunks],
            axis=0
        ).numpy()
...
def transcribe_static_file():
    ...
    voice_wav = apply_yamnet_filter(wav_path, speech_threshold=0.1)
