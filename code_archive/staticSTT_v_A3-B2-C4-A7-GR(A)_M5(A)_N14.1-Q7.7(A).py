...
def find_ahk_pid(script_name):
    """
    Return the PID of the first AutoHotkey process whose command line contains 'script_name'.
    If not found, return None.
    """
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            # Check if the process name is AutoHotkey or AutoHotkeyU64 (depending on your system)
            if proc.info['name'] in ('AutoHotkey.exe', 'AutoHotkeyU64.exe', 'AutoHotkeyU32.exe'):
...
def graceful_exit(sock):
    ...
    # Try killing the known AHK PID first
    if ahk_pid is not None:
        ...
        # No stored PID? Fall back to scanning
        console.print("[yellow]No stored AHK PID. Attempting fallback scanning...[/yellow]")
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            if proc.info['name'] in ('AutoHotkey.exe', 'AutoHotkeyU64.exe', 'AutoHotkeyU32.exe'):
                cmdline_str = " ".join(proc.info['cmdline'])
                if hotkey_script in cmdline_str:
                    console.print(f"[bold red]Killing AHK script by scanning (PID={proc.info['pid']})...[/bold red]")
                    proc.kill()
...
def startup():
    ...
    existing_pid = find_ahk_pid(hotkey_script)
    if existing_pid:
        console.print("[yellow]AHK script already running. Skipping launch.[/yellow]")
        ahk_pid = existing_pid
        console.print(f"[yellow]Stored existing AHK PID: {ahk_pid}[/yellow]")
    else:
        ...
        ahk_pid = proc.pid
        console.print(f"[green]Launched AHK with PID={ahk_pid}[/green]")
