...
def generate_trimmed_audio(input_wav: str, output_wav: str, vad_mode=1):
    ...
    try:
        ...
        console.print(f"[bold red]Failed to open {input_wav} for trimming: {e}[/bold red]")
        return False
            ...
            if dur < MIN_SILENCE_TRIM_SEC:
                ...
                if has_prev_speech:
                    if len(dat) >= two_sec_bytes:
                        ...
                        to_keep_front = dat  # if the block is just a bit over 4s, we keep the entire block in front?
                ...
                # If neither next nor previous speech, block is effectively in isolation
                # but that means we had no speech blocks at all? Actually we do have speech
                # in this chunk or we'd have returned. So probably skip if no adjacency.
                if final_silence:
    ...
    try:
        ...
        console.print(f"[bold red]Failed to write trimmed audio to {output_wav}: {e}[/bold red]")
        return False
    ...
    try:
        while recording:
            ...
            if elapsed_in_chunk >= MIN_CHUNK_LENGTH_SEC:
                ...
                # If enough consecutive silent frames => finalize chunk
