...
# Optional dependencies for real-time transcription
try:
...
class RealtimeTranscriptionHandler:
    def __init__(self, config: Config, console: Console, transcriber: Transcriber, tray: TrayManager):
        ...
        if not REALTIMESTT_AVAILABLE:
            self.console.print("[bold red]RealtimeSTT library not available. Please install with 'pip install RealtimeSTT'[/bold red]")
            return
    ...
    def start(self):
        ...
        if not REALTIMESTT_AVAILABLE:
            self.console.print("[bold red]RealtimeSTT library not available.[/bold red]")
            return
        ...
        # Initialize the recorder with the appropriate audio source
        try:
            self.recorder = AudioToTextRecorder(
                model="large-v3",
                language=self.config.language,
                use_microphone=(not self.config.use_system_audio),
                input_device_index=self.config.input_device_index if self.config.use_system_audio else None
            )
            ...
            self.console.print(f"[bold red]Failed to start real-time transcription: {e}[/bold red]")
            self.is_running = False
