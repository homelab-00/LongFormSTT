...
def toggle_language():
    ...
    console.print("[yellow]Re-initializing the model with new language parameter...[/yellow]")
    
    # unload the current model
    model = None
    
    # re-load the same model_id, but we keep the new 'language' for future transcribe calls
    # (The 'faster-whisper' model is multilingual, so reloading is optional, 
    # but if you specifically want a brand-new load, do it here:)
    model = WhisperModel(
        model_id,
        device=device,
