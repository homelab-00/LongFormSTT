...
# for real-time transcription (a few errors, faster)
# 
# - Due to the fact that the 'turbo' model doesn't support translation,
# - we're passing that job to the LongFormSTT model.
# ---------------------------------------------------------------------
...
class UnifiedConfigDialog:
    def __init__(self, config: Config, console: Console, realtime_handler):
        self.config = config
        self.console = console
    ...
    def _get_available_models(self):
        ...
        if os.path.exists(cache_dir):
            ...
            for model_dir in glob.glob(os.path.join(cache_dir, "**/snapshots/*"), recursive=True):
                if os.path.isdir(model_dir):
                    # Extract model name from directory path
                    parts = model_dir.split(os.sep)
                    if len(parts) >= 5:
                        model_name = parts[-3]
                        if "whisper" in model_name.lower() and model_name not in models:
    ...
    def show_dialog(self):
        ...
        model_tab = tk.Frame(notebook, bg=bg_color)
        ...
        # Model Tab
        model_frame = tk.Frame(model_tab, bg=bg_color)
        ...
        current_model_label.pack(pady=(0, 20))
        
        # Model dropdown
        model_var = tk.StringVar(value=self.realtime_handler.realtime_model_name)
        ...
        enter_frame = tk.Frame(model_frame, bg=bg_color)
        ...
        def apply_selections():
            ...
            result["realtime_model_name"] = model_var.get()
...
class STTApp:
    def __init__(self):
        ...
        self.config_dialog = UnifiedConfigDialog(self.config, self.console, self.realtime_handler)
