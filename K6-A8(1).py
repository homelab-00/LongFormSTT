...
class Transcriber:
    def __init__(self, config: Config, console: Console):
        ...
        self.model_id = "Systran/faster-whisper-large-v3"  # Main model for Long Form STT
...
class StaticFileProcessor:
    ...
    def _transcribe_in_thread(self, file_path: str) -> None:
        ...
        try:
            ...
            # Use longform_language for static file transcription    
            final_text = self.transcriber.transcribe(voice_wav, use_realtime_language=False)
...
class RealtimeTranscriptionHandler:
    def __init__(self, config: Config, console: Console, transcriber: Transcriber, tray: TrayManager):
        ...
        self.initial_prompt_realtime = None
    ...
    def _load_realtime_model(self):
        ...
        if not self.realtime_model_loaded:
            ...
            try:
                ...
                # Ensure we're loading the correct model for real-time transcription
                ...
                # Validate it's different from the main model
                if self.realtime_model_name == self.transcriber.model_id:
                    self.console.print("[bold yellow]Warning: Real-time model is same as main model. This may impact performance.[/bold yellow]")
            except Exception as e:
    ...
    def stop(self):
        ...
        try:
            ...
            self.console.print(f"[red]Error cleaning up audio resources: {e}[/red]")
            # Continue despite errors to ensure full cleanup
        ...
        if self.thread and self.thread.is_alive():
            try:
                ...
                if self.thread.is_alive():
                    ...
                    self.console.print("[yellow]Transcription thread taking longer than expected to stop. Continuing anyway.[/yellow]")
            except Exception as e:
                self.console.print(f"[red]Error waiting for transcription thread: {e}[/red]")
            self.thread = None
        ...
        try:
            ...
            self.console.print(f"[red]Error updating tray icon: {e}[/red]")
    ...
    def _transcription_loop(self):
        ...
        try:
                        ...
                        if self.is_speech_active:
                            ...
                            if (current_time - silence_start_time) * 1000 >= self.silence_threshold_ms:
                                ...
                                if current_segment:
                                    ...
                                    # If real-time language is not English or Greek, always translate to English
                                    ...
                                    if self.realtime_model_loaded:
                                        try:
                                            segments, info = self.realtime_model.transcribe(
                                                ...
                                                initial_prompt=self.initial_prompt_realtime if hasattr(self, 'initial_prompt_realtime') else None,
                                                suppress_tokens=self.suppress_tokens if hasattr(self, 'suppress_tokens') else [-1],
           ...
           # Handle keyboard interrupt gracefully, don't exit the script
           self.console.print("[yellow]Keyboard interrupt detected in real-time transcription. Stopping gracefully.[/yellow]")
           self.is_running = False
        except Exception as e:
           ...
           # Log the full stack trace for debugging
            ...
            try:
               ...
               self.console.print(f"[red]Error during audio cleanup: {cleanup_error}[/red]")
           
           # Always ensure we reset state properly
            self.is_running = False
            self.console.print("[yellow]Real-time transcription loop has ended.[/yellow]")
