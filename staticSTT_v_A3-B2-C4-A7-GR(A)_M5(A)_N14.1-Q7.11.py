...
# [MOD] For noise reduction
# NOTE: Requires installing noisereduce, librosa, and soundfile:
...
# --------------------------------------------------------------------------------------
# [MOD] New function for optional noise reduction
# --------------------------------------------------------------------------------------
def noise_reduce_file(in_wav_path, aggressive=1.5):
    """
    Applies spectral noise reduction via noisereduce library, moderate aggression.
    Returns path to new WAV or None on failure.
    """
    if not NOISE_REDUCTION_AVAILABLE:
        console.print("[red]Noise reduction dependencies not installed. Skipping noise removal.[/red]")
        return in_wav_path  # fallback to original

    out_wav_path = os.path.join(script_dir, "static_temp_denoised.wav")

    try:
        console.print("[yellow]Applying moderate noise reduction...[/yellow]")
        data, sr = librosa.load(in_wav_path, sr=None)
        # n_std_thresh_stationary sets how aggressive the reduction is (default ~1.5)
        reduced_data = nr.reduce_noise(y=data, sr=sr, n_std_thresh_stationary=aggressive)
        sf.write(out_wav_path, reduced_data, sr)
        console.print("[yellow]Noise reduction done.[/yellow]")
        return out_wav_path
    except Exception as e:
        console.print(f"[red]Noise reduction failed: {e}[/red]")
        return in_wav_path  # fallback
...
def transcribe_static_file():
    ...
    Converts to the correct format if needed, then applies a moderate noise removal.
    Finally transcribes via model.
    """
    ...
    # [MOD] Apply noise reduction
    denoised_wav = noise_reduce_file(wav_path, aggressive=1.5)
    if not denoised_wav or not os.path.exists(denoised_wav):
        console.print("[bold red]Could not obtain a noise-reduced WAV. Will try original WAV.[/bold red]")
        denoised_wav = wav_path
    ...
    try:
        ...
        segments, info = model.transcribe(denoised_wav, language=language, task=task)
