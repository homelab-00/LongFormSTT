...
# [MOD] Silence removal (using pydub)
#   pip install pydub
...
# --------------------------------------------------------------------------------------
# Convert arbitrary audio to WAV (16k, mono) if needed
# --------------------------------------------------------------------------------------
def ensure_wav_format(input_path):
    ...
    temp_wav = os.path.join(script_dir, "static_temp_file.wav")
...
# --------------------------------------------------------------------------------------
# [MOD] Function: Remove extended silence for static file
# --------------------------------------------------------------------------------------
def remove_silences_from_wav(in_wav_path, min_silence_len=1000, silence_thresh=-50, keep_silence=200):
    """
    Removes extended periods of silence from a WAV file using pydub.
    ...
    out_wav_path = os.path.join(script_dir, "static_temp_silence_removed.wav")

    try:
        console.print("[yellow]Removing extended silence in static file...[/yellow]")
        audio = AudioSegment.from_wav(in_wav_path)
        # You can tweak the parameters below as needed:
        chunks = split_on_silence(
            audio,
            min_silence_len=min_silence_len,   # 1000 ms of silence
            silence_thresh=silence_thresh,     # -50 dB is moderate threshold
            keep_silence=keep_silence          # keep 200ms of leading/trailing silence for each chunk
        )
        ...
        console.print("[yellow]Silence removal done. Result saved.[/yellow]")
        return out_wav_path
    except Exception as e:
...
def transcribe_static_file():
    ...
    Converts to the correct format if needed, then removes extended silence.
    ...
    # [MOD] Remove extended silence
    silence_removed_wav = remove_silences_from_wav(wav_path, min_silence_len=1000, silence_thresh=-50, keep_silence=200)
    if not silence_removed_wav or not os.path.exists(silence_removed_wav):
        console.print("[bold red]Silence removal failed or returned nothing. Will try original WAV.[/bold red]")
