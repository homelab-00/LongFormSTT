...
class LanguageSelector:
    ...
    def show_dialog(self):
        ...
        # Frame for search
        search_frame = tkinter.Frame(root, bg="#f0f0f0")
        search_frame.pack(pady=10, padx=20, fill="x")
        
        # Search label
        search_label = tkinter.Label(search_frame, text="Search Language:", bg="#f0f0f0", font=("Arial", 12))
        ...
        search_entry = tkinter.Entry(search_frame, textvariable=search_var, font=("Arial", 12), width=30)
        search_entry.pack(fill="x", pady=5)
        search_entry.focus_set()
        
        # Listbox for languages
        listbox_frame = tkinter.Frame(root, bg="#f0f0f0")
        ...
        # Create buttons
        button_frame = tkinter.Frame(root, bg="#f0f0f0")
...
class RealtimeTranscriptionHandler:
    def __init__(self, config: Config, console: Console, transcriber: Transcriber, tray: TrayManager):
        ...
        # Audio buffers
        self.audio_buffer = collections.deque(maxlen=50)  # Adjust buffer size as needed
        self.frames = []
        ...
        try:
            ...
            while self.is_running and not self.stop_event.is_set():
                ...
                try:
                    ...
                    # Process accumulated audio every few seconds
                    current_time = time.time()
                    if current_time - last_transcription_time > 2.0:  # Adjust interval as needed
                        if accumulated_audio:
                            # Convert to numpy array for processing
                            audio_data = np.frombuffer(b''.join(accumulated_audio), dtype=np.int16)
                            ...
                            # Keep a portion of the audio for context in the next transcription
                            # (similar to a sliding window)
                            keep_frames = len(accumulated_audio) // 2
                            accumulated_audio = accumulated_audio[-keep_frames:] if keep_frames > 0 else []
...
class STTApp:
    ...
    def open_language_menu(self) -> None:
        ...
        if selected_language:
            ...
            self.console.print(f"[yellow]Language changed from {old_lang} to {selected_language} ({language_name})[/yellow]")
