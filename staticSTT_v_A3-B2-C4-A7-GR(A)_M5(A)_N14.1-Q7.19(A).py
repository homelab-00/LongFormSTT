# NOTES:
# - When transcribing static files, the presence of music e.g. a jingle even as short at 5s can cause the script to 'lose' a lot of seconds of speech after that.
...
# [MOD] We'll remove the old pydub silence removal.
# Instead, we'll use webrtcvad to remove non-voice (like music). That means only voice frames pass.
...
def cleanup_static_temp():
    candidates = [
        ...
        os.path.join(script_dir, "temp_static_silence_removed.wav"),  # [MOD] we'll use that for VAD
    ]
    for f in candidates:
        if os.path.exists(f):
            try:
...
# --------------------------------------------------------------------------------------
# [MOD] WebRTC VAD approach to remove silence
# We'll call it 'apply_vad_to_wav' -> 'temp_static_silence_removed.wav'
# --------------------------------------------------------------------------------------
def apply_vad_to_wav(in_wav_path, aggressiveness=2):
    """
    Use webrtcvad to keep only frames that are likely voice.
    Return path to new file: temp_static_silence_removed.wav.
    If webrtcvad not installed, or error, return original.
    """
    if not WEBRTC_VAD_AVAILABLE:
        console.print("[red]webrtcvad not installed. Skipping VAD.[/red]")
        return in_wav_path

    out_wav = os.path.join(script_dir, "temp_static_silence_removed.wav")

    try:
        # 1) open input, read frames.
        wf_in = wave.open(in_wav_path, 'rb')
        channels = wf_in.getnchannels()
        rate = wf_in.getframerate()
        if channels != 1:
            console.print("[red]VAD requires mono, got more channels. Skipping VAD.[/red]")
            wf_in.close()
            return in_wav_path
        if rate not in [8000,16000,32000,48000]:
            console.print("[red]VAD only works with specific rates. Skipping VAD.[/red]")
            wf_in.close()
            return in_wav_path
        ...
        # 2) create a VAD with given aggressiveness.
        vad = webrtcvad.Vad(aggressiveness)

        # We will chunk the audio_data into small frames of 30 ms each.
        frame_ms = 30
        frame_bytes = int(rate * 2 * (frame_ms/1000.0))  # for 16-bit
        # e.g. at 16000, 30ms is 16000 * (30/1000)=480 samples => 960 bytes

        voiced_bytes = bytearray()
        idx = 0
        # We'll handle the case if there's leftover at the end.

        while idx + frame_bytes <= len(audio_data):
            frame = audio_data[idx:idx+int(frame_bytes)]
            is_speech = vad.is_speech(frame, rate)
            if is_speech:
                voiced_bytes.extend(frame)
            idx += int(frame_bytes)

        # 3) write the kept frames.
        if len(voiced_bytes)==0:
            console.print("[red]VAD found no voice frames, returning original.[/red]")
            return in_wav_path
        ...
        console.print("[yellow]VAD kept voice frames => saved temp_static_silence_removed.wav[/yellow]")
        return out_wav
    except Exception as e:
...
def transcribe_static_file():
    ...
    # 1) Convert to 16k/mono if needed => temp_static_file.wav
    ...
    # 2) apply VAD => temp_static_silence_removed.wav
    #    Remove silence.
    voice_wav = apply_vad_to_wav(wav_path, aggressiveness=2)

    # 3) Transcribe final file
    try:
        console.print("[blue]Beginning static transcription with voice-only data...[/blue]")
