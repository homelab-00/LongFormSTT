...
class StaticFileProcessor:
    def __init__(self, config: Config, console: Console, transcriber: Transcriber, tray: TrayManager):
        ...
        self.transcription_thread = None  # Track the transcription thread
        self.static_transcription_lock = threading.Lock()  # Add lock for thread safety
    ...
    def request_abort(self) -> None:
        ...
        self.console.print("[yellow]Static transcription abort requested.[/yellow]")
        self.console.print("[yellow]Transcription will be aborted when the current operation completes.[/yellow]")
        
        # Flash the tray icon white to indicate a pending abort
        self.tray.set_color('white', self.config.send_enter)
        
        # Schedule changing it back after 0.5 seconds
        def revert_icon():
            time.sleep(0.5)
            with self.static_transcription_lock:
    ...
    def _transcribe_in_thread(self, file_path: str) -> None:
        ...
        try:
            ...
            # This is a blocking call and we don't have a good way to interrupt it
