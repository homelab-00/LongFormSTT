    ...
    try:
        with wave.open(input_path, 'rb') as wf:
            ...
            if channels == 1 and rate == 16000:
                # Already correct. Just copy to temp.
...
def remove_silences_from_wav(in_wav_path, min_silence_len=1000, silence_thresh=-50, keep_silence=200):
    ...
    try:
        ...
        chunks = split_on_silence(
            audio,
            min_silence_len=min_silence_len,  # default 1000 ms
            silence_thresh=silence_thresh,    # -50 dB is moderate threshold
            keep_silence=keep_silence         # keep 200ms of leading/trailing silence in each chunk
        )
...
def transcribe_static_file():
    ...
    silence_removed_wav = remove_silences_from_wav(
        wav_path,
        min_silence_len=1000,  # moderate length
        silence_thresh=-50,    # moderate threshold in dB
        keep_silence=200       # keep 200ms around chunks
    )
